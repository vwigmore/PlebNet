...
from plebnet.cmd_parser import cmd_irc
...
def execute_irc(cmd=sys.argv[2:]):
    ...
    subparsers = parser.add_subparsers(dest="command")
    parser_list = subparsers.add_parser("status", help="plebnet en dan irc en dan status")
    parser_list.set_defaults(func=execute_irc_status)

    parser_list = subparsers.add_parser("stop", help="plebnet en dan irc en dan stop")
    parser_list.set_defaults(func=execute_irc_stop)

    args = parser.parse_args(cmd)
    args.func(args)


def execute_irc_status(args):
    cmd_irc.status_irc_client()


def execute_irc_stop(args):
    cmd_irc.stop_irc_client()


# def add_parser_irc(subparsers):
#     parser = subparsers.add_parser("irc", help="irc you made it!")
#
#     subparser1 = parser.add_argument("stat")
#
#     parser_list = subparser1.add_parser("status", help="")
